    ...
    private List<CalCalendar> getCalendarsWithoutHoliday(List<CalCalendar> days){
        ...
        if(CollUtil.isNotEmpty(holidays)){
            List<CalCalendar> results = days.stream().filter(
                    calCalendar -> holidays.stream()
                            .filter(calHoliday -> "HOLIDAY".equals(calHoliday.getHolidayType()))
                            .noneMatch(calHoliday -> calCalendar.getTheDay().equals(CalendarUtil.getDateStr(calHoliday.getTheDay())))
