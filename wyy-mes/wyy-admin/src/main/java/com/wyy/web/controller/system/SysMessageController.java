...
import com.wyy.framework.message.MessageProvider;
...
public class SysMessageController extends BaseController
{
    ...
    /**
     * 消息标记为已读（不传 messageId 时标记当前用户全部消息已读）
     */
    @PreAuthorize("@ss.hasPermi('system:message:list')")
    @Log(title = "消息标记已读", businessType = BusinessType.UPDATE)
    @GetMapping("/read")
    public AjaxResult read(Long messageId)
    {
        Long userId = com.wyy.common.utils.SecurityUtils.getUserId();
        if (messageId != null)
        {
            SysMessage message = sysMessageService.selectSysMessageByMessageId(messageId);
            if (message == null || !userId.equals(message.getRecipientId()))
            {
                return AjaxResult.error("消息不存在或无权操作");
            }
        ...
        else
        {
            ...
            List<SysMessage> messages = sysMessageService.selectSysMessageList(query);
