...
public class WmBarcodeController extends BaseController
{
     ...
     * 根据业务对象查询条码信息。
     *
     * 条码预览组件会以业务ID、业务编码和条码类型调用该接口。必须使用
     * 明确的静态路径，否则会被下方的 /{barcodeId} 路由当成数字主键处理。
     */
    @PreAuthorize("@ss.hasPermi('mes:wm:barcode:query')")
    @GetMapping("/getBarcodeUrl")
    public AjaxResult getBarcodeUrl(WmBarcode wmBarcode)
    {
        List<WmBarcode> barcodes = wmBarcodeService.selectWmBarcodeList(wmBarcode);
        return AjaxResult.success(barcodes.isEmpty() ? null : barcodes.get(0));
    }
