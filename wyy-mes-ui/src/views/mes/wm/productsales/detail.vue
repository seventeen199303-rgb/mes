<template>
  <div class="app-container">    
    <el-table v-loading="loading" :data="productsaleslineList">
      ...
      <el-table-column label="仓库" width="160px" align="center" prop="warehouseName" :show-overflow-tooltip="true" />
      <el-table-column label="库区" width="140px" align="center" prop="locationName" :show-overflow-tooltip="true" />
      <el-table-column label="库位" width="160px" align="center" prop="areaName" :show-overflow-tooltip="true" />
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-row>
            <el-col :span="8">
              ...
              <StockSelect ref="stockSelect" :itemId="form.itemId" :batchId="form.batchId" @onSelected="onStockSelected"></StockSelect>
            </el-col>
            <el-col :span="8">
...
<script>
import { getProductsalesdetail, delProductsalesdetail, addProductsalesdetail, updateProductsalesdetail } from "@/api/mes/wm/productsalesdetail";
import { listWithDetail } from "@/api/mes/wm/productsalesline";
...
export default {
  ...
  methods: {
    ...
    reset() {
      this.form = {
        detailId: null,        lineId: null,        salseId: this.salseId,        materialStockId: null,        itemId: null,        itemCode: null,        itemName: null,        specification: null,        unitOfMeasure: null,        unitName: null,        quantity: 0,        batchId: null,        batchCode: null,        warehouseId: null,        warehouseCode: null,        warehouseName: null,        locationId: null,        locationCode: null,        locationName: null,        areaId: null,        areaCode: null,        areaName: null,        remark: null,        attr1: null,        attr2: null,        attr3: null,        attr4: null,        createBy: null,        createTime: null,        updateBy: null,        updateTime: null      };
      this.resetForm("form");
    },
    /** 搜索按钮操作 */
    handleQuery() {
    ...
    handleAdd(row) {
      ...
      this.open = true;
      this.title = "添加产品销售出库记录明细";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const detailId = row.detailId || this.ids
      getProductsalesdetail(detailId).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改产品销售出库记录明细";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.detailId != null) {
            updateProductsalesdetail(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getListWithDetail();
            });
          } else {
            addProductsalesdetail(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除产品销售出库记录明细编号为"' + detailIds + '"的数据项？').then(function() {
        return delProductsalesdetail(detailIds);
      }).then(() => {
        this.getListWithDetail();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('wm/productsalesdetail/export', {
        ...this.queryParams
      }, `productsalesdetail_${new Date().getTime()}.xlsx`)
    },
