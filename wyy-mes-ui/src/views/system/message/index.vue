<template>
  <div class="app-container">
    ...
    <!-- 消息详情对话框 -->
    <el-dialog :title="viewTitle" :visible.sync="viewOpen" width="620px" append-to-body>
      <el-descriptions :column="1" border size="medium">
        <el-descriptions-item label="消息类型"><dict-tag :options="dict.type.sys_message_type" :value="viewMessage.messageType"/></el-descriptions-item>
        <el-descriptions-item label="消息级别"><dict-tag :options="dict.type.sys_message_level" :value="viewMessage.messageLevel"/></el-descriptions-item>
        <el-descriptions-item label="标题">{{ viewMessage.messageTitle }}</el-descriptions-item>
        <el-descriptions-item label="内容">{{ viewMessage.messageContent }}</el-descriptions-item>
        <el-descriptions-item label="发送人">{{ viewMessage.senderNick || viewMessage.senderName || '系统' }}</el-descriptions-item>
        <el-descriptions-item label="接收人">{{ viewMessage.recipientNick || viewMessage.recipientName || '-' }}</el-descriptions-item>
        <el-descriptions-item label="状态"><dict-tag :options="dict.type.sys_message_status" :value="viewMessage.status"/></el-descriptions-item>
        <el-descriptions-item label="发送时间">{{ parseTime(viewMessage.createTime) }}</el-descriptions-item>
      </el-descriptions>
      <div slot="footer" class="dialog-footer">
        <el-button v-if="viewMessage.status === 'UNREAD'" type="primary" @click="markRead(viewMessage)">标记已读</el-button>
...
<script>
import {listMessage, getMessage, delMessage, addMessage, updateMessage, allRead, markMessageRead} from "@/api/system/message";
...
export default {
  ...
  computed: {
    viewTitle() {
      return this.viewMessage.messageId ? `消息详情：${this.viewMessage.messageTitle || ''}` : '消息详情'
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    /** 标记单条消息已读 */
    markRead(row) {
      markMessageRead(row.messageId).then(() => {
        row.status = 'READ';
        this.viewMessage = { ...row };
        this.getList();
      });
